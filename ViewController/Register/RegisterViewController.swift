...
class RegisterViewController: UIViewController {
    ...
    var userStoreData: [addressDataByKeyworld] = []
    ...
    @IBAction func submitButtonPressed(_ sender: UIButton) {
        ...
        selectedData.category = categoryButton.titleLabel?.text
        ...
        if titleLabel.text == "" || addressLabel.text == "" || categoryButton.titleLabel?.text == "카테고리 선택" {
            showToast(message: "등록되지 않은 필드가 존재 합니다.")
            drawIndicator(activityIndicator: activityIndicator, isActive: false)
            submitButton.isHidden = false
            return
        ...
        self.checkStore(storeID: self.storeID)
        print("self.storeID : ", self.storeID)
        print("storeIDAvailFlag : ", storeIDAvailFlag)
        ...
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 3) {
            if self.storeIDAvailFlag { // 등록된 이력이 없을 경우
                ...
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 3) {
                    ...
                    if !self.selectedFromRestarantFlag {
                        // 이동전 등록된 데이터 삭제
                    ...
                    self.selectedFromRestarantFlag = false
                ...
                if self.savedImage + self.images.count > 5 {
                    self.showToast(message: "사진은 최대 5장까지만 저장이 가능합니다. 현재: \(self.savedImage)")
                    self.drawIndicator(activityIndicator: self.activityIndicator, isActive: false)
                    return
                }
                ...
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 3) {
                    self.selectedData.photo = self.urls
                    print("selectedData.photo : ", self.selectedData.photo)
            ...
            case 200:
                ...
                if result == "SUCCESS" {
                    ...
                    for item in data {
                        ...
                        self.userStoreData.append(data)
                    }
                } else {
    ...
    func checkStore(storeID: Int?) {
        ...
        if selectedFromRestarantFlag {
            for i in 0 ..< userStoreData.count {
                if userStoreData[i].phone == selectedData.phone && userStoreData[i].place_url == selectedData.place_url {
                    self.storeID = storeIdField[i]
                    self.savedImage = userStoreData[i].photo?.count ?? 0
                    break;
                }
            }
        } else {
            if userStoreData.count != 0 {
                for i in 0 ..< userStoreData.count {
                    print("i : ", i)
                    if selectedData.phone == userStoreData[i].phone && selectedData.place_url == userStoreData[i].place_url {
                        self.storeID = storeIdField[i]
                        break;
                    }
                }
        ...
        for item in storeIdField {
            if Int(item) == self.storeID {
                print("in?")
                self.storeIDAvailFlag = false
                break;
            }
    ...
    func updateStoreData(storeID: Int) {
        ...
        HungryAPIManager.shared.putStoreDataByIndex(storeData: selectedData, storeIndex: storeID, userIndex: UserDefaultManager.shared.user.index) { code, json in
            ...
            print("updateStoreData result : ", result)
            print(json)
